        ...
        for (RecordMissingReport e : recordMissing) {
            for (TransactionDetailReport x : transactionDetail) {
                if (e.getPhoneNumber().equals(x.getPhoneNumber())) {
                    missingRecordTransTime.add(new RecordMissingTransactionTime(x.getPhoneNumber(), x.getCloseTransTime(), LocalDateTime.parse(x.getCloseTransTime(), inputFormatter)));
                }
            }
        }
